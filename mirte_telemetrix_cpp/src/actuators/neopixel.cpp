...
Neopixel::get_neopixels(NodeData node_data, std::shared_ptr<Parser> parser) {
  std::vector<std::shared_ptr<TelemetrixDevice>> neopixels;
   auto modules = parser->update_params_list_type("modules", "neopixel_module_names", "neopixel");
parser->fix_param_type_str_modules("modules", modules, {"pins.data"});
  auto param_listener =
      std::make_shared<mirte_telemetrix_cpp_neopixel::ParamListener>(parser->nh);
  ...
  auto datas =
      ...
      std::views::transform([&](const auto &pair) {
        ...
        parameters["default_r"] = rclcpp::ParameterValue(map_neo.default_color.r);
        parameters["default_g"] = rclcpp::ParameterValue(map_neo.default_color.g);
        parameters["default_b"] = rclcpp::ParameterValue(map_neo.default_color.b);
     ...
     for (auto data : datas) {
        if(neopixels.size() >= 1){
          RCLCPP_ERROR(parser->nh->get_logger(), "Multiple neopixel modules found, only one is supported, skipping module with name: %s", data.name.c_str());
          break;
        }
...
Neopixel::Neopixel(NodeData node_data, NeopixelData neopixel_data)
  ...
  node_data.tmx->attach_neopixel(neopixel_data.data_pin, neopixel_data.num_leds, 0, {neopixel_data.default_r, neopixel_data.default_g, neopixel_data.default_b});
        ...
        this->set_neopixel_single_service = nh->create_service<mirte_msgs::srv::SetNeopixelSingle>(
...
void Neopixel::set_color_single_service(
  ...
  if(req->led_index < 0 || req->led_index >= this->num_leds){
    RCLCPP_ERROR(this->nh->get_logger(), "LED index out of range: %d", req->led_index);
    res->status = false;
    return;
  }
  this->tmx->set_neopixel_color(req->led_index, {req->color.r, req->color.g, req->color.b}, true);
  res->status = true;
}
