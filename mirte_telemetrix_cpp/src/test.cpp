...
//     //   search for dynamic parameters, e.g., parameters that start with
//     prefix_ + "modules.", then extract the module name
//       auto x = parameters_interface_->list_parameters({}, 10).names |
//       std::views::transform([this](const std::string& param_name){
...
//                     word_regex, ""); auto next_key = replaced.substr(0,
//                     replaced.find('.')); return next_key;
//                 } else {
//                     return std::string{};
//                 }
...
//       ; for (const auto& v : x) {
//         std::cout << "Found dynamic parameter module name: " << v <<
//         std::endl;
//           if (std::find(updated_params.module_names.begin(),
//           updated_params.module_names.end(), v) ==
//           updated_params.module_names.end()) {
//             std::cout << "Adding dynamic parameter module name: " << v <<
//             std::endl;
//               updated_params.module_names.push_back(v);
//               std::cout << "size now: " << updated_params.module_names.size()
//               << std::endl;
//               parameters_interface_->set_parameters({rclcpp::Parameter(fmt::format("{}{}",
//               prefix_, "module_names"), updated_params.module_names)});
//           }
//       }

//   END EDITTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
bool update_params_list(
    std::string const &prefix_, std::string const &list_name,
    const std::shared_ptr<rclcpp::node_interfaces::NodeParametersInterface>
        &parameters_interface_) {
  // __map_xxxxx does not automatically update the list of names, so we need to
  // do it here
  //  auto x = parameters_interface_->list_parameters({prefix_}, 10).names |
  //  std::views::transform([prefix_](const std::string& param_name){
  //       // std::cout << "Checking parameter: " << param_name << std::endl;
  ...
  //       << std::endl; vec_x.insert(v);
  //     }
  //     auto module_names = std::vector<std::string>(vec_x.begin(),
  //     vec_x.end()); std::cout << "Final module "<< list_name << " list: ";
  //     for(const auto& name : module_names) {
  //         std::cout << name << " ";
  //     }
  //     std::cout << std::endl;
  //     parameters_interface_->set_parameters({rclcpp::Parameter(list_name,
  //     module_names)});
  return true;
}
bool update_params_list(
  ...
  //                   replaced.find('.'));
  //                   // std::cout << "  extracted key: " << next_key <<
  //                   std::endl; return std::pair(param_name, next_key);
  //               } else {
  //                   return std::pair<std::string, std::string>{};
  //               }

  //       //   return param.get_name().starts_with(prefix_ + "modules.");
  //     }) | std::views::filter([](const std::pair<std::string, std::string>&
  //     s){ return !s.second.empty(); }) |
  //     std::views::filter([filter_func](const std::pair<std::string,
  //     std::string>& s){
  //         return filter_func(s.first);
  //     })
  //     | std::views::transform([](const std::pair<std::string, std::string>&
  //     s){
  //         return s.second;
  //     });
...
void update_params_list_type(
  ...
  update_params_list(
      prefix_, list_name, parameters_interface_, [&](const std::string &name) {
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    // param_listener2->parameters_interface_->set_parameters({rclcpp::Parameter("module_names",
    // std::vector<std::string>{"ina_module_1", "ina_module_2"})});
    if (update_params_list("modules", "module_names",
                           node->get_node_parameters_interface())) {

      // param_listener2->refresh_dynamic_parameters();   //
      // param_listener2->update_internal_params(params2);
    }
    update_params_list_type("modules", "ina_module_names",
                            node->get_node_parameters_interface(), "INA226");
    //     // check if ends with type, then check if param type == "INA226"
    //     std::cout << "Filtering parameter: " << name << std::endl;
    //     if(name.size() < 5) {
    //         std::cout << "  too short" << std::endl;
    ...
    // std::cout <<
    // params2.modules.ina_module_names_map.begin()->second.power_low_time <<
    // std::endl;
