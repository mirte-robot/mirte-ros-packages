...
SonarMonitor::get_sonar_monitors(NodeData node_data,
  ...
  auto sonars =
      ...
      std::views::transform([&](const auto &pair) {
        ...
        parameters["unit"] = rclcpp::ParameterValue(map_distance.unit);
        std::set<std::string> unused_keys = get_keys(parameters);

        return SonarData(parser, node_data.board, name, parameters, unused_keys,
                         map_distance);
      }) |
...
void SonarMonitor::data_callback(uint16_t value) {
  ...
  if (value == 0xFFFF) {
    ...
    if (raw_distance < min_range) {
      ...
      this->distance = raw_distance / sonar_data.unit;
    }
  }
}
