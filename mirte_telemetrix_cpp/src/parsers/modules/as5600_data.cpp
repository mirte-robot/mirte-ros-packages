...
if(unused_keys.erase("encoders")) {
  ...
  if (parameters.count("encoders")) {
    ...
    auto encoders = parser->get_params_keys(parser->build_param_name(get_device_key(this), "encoders"));
    std::cout << "AS5600 encoders: " << encoders.size() << std::endl;

    for (const auto &encoder : encoders) {
      std::cout << "AS5600 encoder: " << encoder << std::endl;
      // auto key = parser->build_param_name(get_device_key(this), "encoders", encoder + ".mux");
      auto key = parser->build_param_name({get_device_key(this), "encoders", encoder ,"mux"});
      std::cout << "AS5600 encoder key: " << key << std::endl;
      if (unused_keys.erase(key) == 0) {
        std::cout << "AS5600 encoder '" << encoder << "' mux not found in parameters" << std::endl;
        // continue;
      }
      auto channel = get_string(parameters[parser->build_param_name({"encoders", encoder, "mux"})]);
      this->encoders.push_back(std::make_tuple(
          std::stoi(channel),
