...
class MirteMasterArm(Node):
    def __init__(self):
        ...
        self.shoulder_client = self.create_client(SetServoAngle, '/io/servo/hiwonder/shoulder_lift/set_angle')
        self.shoulder_pan_client = self.create_client(SetServoAngle, '/io/servo/hiwonder/shoulder_pan/set_angle')
        self.elbow_client = self.create_client(SetServoAngle, '/io/servo/hiwonder/elbow/set_angle')
        self.wrist_client = self.create_client(SetServoAngle, '/io/servo/hiwonder/wrist/set_angle')
        self.gripper_action_client = ActionClient(self, GripperCommand, '/mirte_master_gripper_controller/gripper_cmd')
    ...
    def joy_calc(self):
        ...
        if shoulder_changed:
            ...
            self.send_servo_request(self.shoulder_client, self.shoulder_angle, 'shoulder')
        ...
        if shoulder_pan_changed:
            self.send_servo_request(self.shoulder_pan_client, self.shoulder_pan_angle, 'shoulder_pan')
    ...
    def calc_shoulder_angles(self, shoulder_angle):
        ...
        if shoulder_angle < -1.66:
            self.elbow_angle = (shoulder_angle + 1.66) * 1.0  # simple linear mapping for demonstration

        else:
            self.elbow_angle = 0.0 #(shoulder_angle + 1.66) * 0.5  # simple linear mapping for demonstration
        
        self.wrist_angle = -self.elbow_angle  # simple inverse relationship for demonstration
